...
work_yaw_pup = rospy.Publisher("/work_yaw", Float64, queue_size=1)

rospy.init_node("work_yaw")
...
def work_yaw(curr_change):
    correction_factor = 0.5
    while(True):
        rospy.set_param("/padel", 0.2)
        rospy.set_param("/steering_angle", 18)
        if(curr_change + correction_factor > 2 * math.pi):
            break
